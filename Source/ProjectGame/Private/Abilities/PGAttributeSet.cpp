...
void UPGAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
    ...
    AProjectBaseCharacter* SourceCharacter = nullptr;
    if (Source && Source->AbilityActorInfo.IsValid() && Source->AbilityActorInfo->AvatarActor.IsValid())
    {
        ...
        if (SourceController)
        {
            SourceCharacter = Cast<AProjectBaseCharacter>(SourceController->GetPawn());
        }
        else
        {
            SourceCharacter = Cast<AProjectBaseCharacter>(SourceActor);
        }
    ...
    if (Data.EvaluatedData.Attribute == GetDamageAttribute())
    {
        ...
        if (LocalDamageDone > 0.0f)
        {
            ...
            {
                // UE_LOG(LogTemp, Warning, TEXT("%s() %s is NOT alive when receiving damage"), TEXT(__FUNCTION__),
            ...
            if (TargetCharacter && WasAlive)
            {
                ...
                // UE_LOG(LogTemp, Log, TEXT("%s() %s Damage Received: %f"), TEXT(__FUNCTION__), *GetOwningActor()->GetName(),
                // LocalDamageDone);
                ...
                if (Hit)
                {
                    ...
                    switch (HitDirection)
                    {
                        case EGDHitReactDirection::Left: TargetCharacter->PlayHitReact(HitDirectionLeftTag, SourceCharacter); break;
                        case EGDHitReactDirection::Front: TargetCharacter->PlayHitReact(HitDirectionFrontTag, SourceCharacter); break;
                        case EGDHitReactDirection::Right: TargetCharacter->PlayHitReact(HitDirectionRightTag, SourceCharacter); break;
                        case EGDHitReactDirection::Back: TargetCharacter->PlayHitReact(HitDirectionBackTag, SourceCharacter); break;
                    }
                }
                else
                {
                ...
                if (!TargetCharacter->IsAlive())
                {
                    ...
                    if (SourceController != TargetController)
                    {
                        ...
                        InfoXP.Attribute = UPGAttributeSet::GetXPAttribute();

                        FGameplayModifierInfo& InfoGold = GEBounty->Modifiers[Idx + 1];
                        InfoGold.ModifierMagnitude = FScalableFloat(GetGoldBounty());
                        InfoGold.ModifierOp = EGameplayModOp::Additive;
                        InfoGold.Attribute = UPGAttributeSet::GetGoldAttribute();
